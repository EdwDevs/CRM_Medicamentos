export const renderTable = (data, { utils, getProductConfig }) => {
  ...
  data.forEach((item, index) => {
    ...
    row.innerHTML = `
      ...
      <td class="col-priority-essential table-cell-money" data-label="Total">${utils.fmtMoney(item.totalAmount)}</td>
      <td class="text-center col-priority-essential" data-label="Estado">
          ...
          <button onclick="app.toggleStatus('${item.id}', '${item.status}')"
              class="btn btn-sm border-0 table-status-btn badge-status ${isProcessed ? "is-procesado" : "is-pendiente"}">
      ...
      <td class="text-end pe-md-4 col-priority-essential" data-label="Acciones">
          <div class="d-flex justify-content-end gap-2 align-items-center">
              ...
              <button onclick="app.openDetails('${item.id}')" class="btn btn-sm table-action-btn btn-table-action" aria-label="Ver detalle de la fila">
                  <i class="fa-regular fa-eye"></i>
              </button>
              <!-- Importante: datos/acciones secundarios se mueven al patrón de expansión por fila. -->
              <details class="row-expand-details">
                  <summary class="btn btn-sm table-action-btn btn-table-action" aria-label="Ver más opciones y datos de la fila">
                  ...
                  <div class="row-expand-panel text-start">
                      ...
                      <div class="pt-2 border-top mt-2 col-priority-secondary">
                          <button onclick="app.deleteItem('${item.id}')" class="btn btn-sm table-action-btn btn-table-action text-danger w-100">
